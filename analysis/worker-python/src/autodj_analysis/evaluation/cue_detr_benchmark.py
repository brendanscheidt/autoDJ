"""Benchmark CUE-DETR cue candidates against Rekordbox drop-start cues."""
...
from ..cue_detr import CueDetrOptions, CueDetrPredictor
...
CUE_DETR_DROP_BENCHMARK_REPORT_TYPE = "cue-detr-drop-anchor-benchmark"
DEFAULT_CUE_DETR_DROP_TOP_K = 5
DEFAULT_CUE_DETR_DROP_MATCH_TOLERANCE_SECONDS = 0.35
DEFAULT_CUE_DETR_SNAP_WINDOW_SECONDS = 0.75


def run_cue_detr_drop_benchmark(
    rekordbox_xml_path: str | Path,
    output_root: str | Path,
    *,
    analysis_root: str | Path | None = None,
    top_k: int = DEFAULT_CUE_DETR_DROP_TOP_K,
    match_tolerance_seconds: float = DEFAULT_CUE_DETR_DROP_MATCH_TOLERANCE_SECONDS,
    snap_window_seconds: float = DEFAULT_CUE_DETR_SNAP_WINDOW_SECONDS,
    cue_detr_options: CueDetrOptions | None = None,
    limit: int | None = None,
) -> dict[str, Any]:
    """Run CUE-DETR over Rekordbox tracks and compare candidates to drop cues."""
    ...
    predictor = CueDetrPredictor(cue_detr_options)
    cases = [
        _run_case(
            track,
            predictor,
    ...
    summary = {
        ...
        "reportType": CUE_DETR_DROP_BENCHMARK_REPORT_TYPE,
        ...
        "parameters": {
            ...
            "limit": limit,
            "cueDetr": (cue_detr_options or CueDetrOptions()).__dict__,
        },
        "cases": cases,
        "summary": _summary(cases, top_k=top_k),
    }
    write_json_atomic(output_root / "cue-detr-drop-benchmark-summary.json", summary)
    return summary


def _run_case(
    track: RekordboxTrack,
    predictor: CueDetrPredictor,
    ...
    *,
    ...
    prediction_path = write_json_atomic(case_dir / "cue-detr-candidates.json", artifact)
    ...
    candidates_path = write_json_atomic(
        case_dir / "cue-detr-drop-candidates.json",
        {
            "schemaVersion": SCHEMA_VERSION,
            "artifact": "cue-detr-drop-candidates",
...
def snap_cue_candidates_to_beat_grid(
    candidates: Sequence[dict[str, Any]],
    beat_grid: Sequence[tuple[int | None, float]],
    *,
    snap_window_seconds: float,
) -> list[dict[str, Any]]:
    """Snap cue candidates to the nearest analyzed beat for DJ-usable anchors."""

    snapped = []
    seen_times: set[float] = set()
    for candidate in sorted(candidates, key=lambda item: (-float(item.get("score", 0.0)), float(item["timeSeconds"]))):
        time_seconds = float(candidate["timeSeconds"])
        output = dict(candidate)
        if beat_grid:
            nearest_index, nearest_time = min(beat_grid, key=lambda beat: abs(beat[1] - time_seconds))
            snap_error = nearest_time - time_seconds
            if abs(snap_error) <= snap_window_seconds:
                output["timeSeconds"] = _round_float(nearest_time)
                output["sourceTimeSeconds"] = _round_float(time_seconds)
                output["snappedToBeatGrid"] = True
                output["snapErrorMilliseconds"] = _round_float(snap_error * 1000.0)
                output["beatIndex"] = nearest_index
            else:
                output["snappedToBeatGrid"] = False
                output["sourceTimeSeconds"] = _round_float(time_seconds)
                output["snapErrorMilliseconds"] = None
                output["beatIndex"] = None
        else:
            output["snappedToBeatGrid"] = False
            output["sourceTimeSeconds"] = _round_float(time_seconds)
            output["snapErrorMilliseconds"] = None
            output["beatIndex"] = None
        dedupe_key = round(float(output["timeSeconds"]), 3)
        if dedupe_key in seen_times:
            continue
        seen_times.add(dedupe_key)
        snapped.append(output)
    for rank, candidate in enumerate(snapped, start=1):
        candidate["rank"] = rank
    return snapped
...
def _summary(cases: Sequence[dict[str, Any]], *, top_k: int) -> dict[str, Any]:
    ...
    return {
        ...
        "errors": [
            {"trackName": case["trackName"], **case["error"]}
            for case in cases
            if case.get("error")
        ],
    }
